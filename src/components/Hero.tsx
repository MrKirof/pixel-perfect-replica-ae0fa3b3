...
const Hero = ({ onStartProject }: { onStartProject?: () => void }) => {
  ...
  const isMobile = config.pixelRatio === 1 && !config.enableAnimatedPlanet;

  useEffect(() => {
    const t = setTimeout(() => setLoaded(true), isMobile ? 0 : 100);
  ...
  return (
    <section className="relative min-h-[100svh] flex flex-col overflow-hidden px-0">
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center sm:justify-end pb-[12vh] sm:pb-[22vh] md:pb-[14vh] pt-20 sm:pt-32">
        <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 md:px-8 lg:px-16">
          ...
          <div className={`mb-5 md:mb-8 transition-all duration-700 ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`} style={{ transitionDelay: isMobile ? "0s" : "0.1s" }}>
            <div className="inline-flex items-center gap-2 border border-white/20 bg-white/[0.08] backdrop-blur-sm px-3 py-2 md:px-5 md:py-2.5 rounded-full">
              <div className="w-2 h-2 rounded-full bg-accent animate-pulse" />
              <span className="font-mono text-[10px] sm:text-xs uppercase tracking-[0.18em] text-white/80">
                Est. 2013 | Remote Agency
              </span>
            </div>
          </div>

          <h1 className="font-display font-extrabold text-left leading-[0.9] tracking-tight mb-5 md:mb-8 max-w-4xl text-[2.2rem] sm:text-4xl md:text-5xl lg:text-6xl">
            <span className="block overflow-hidden">
              <span className={`block transition-all duration-700 ease-out text-white ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-full"}`} style={{ transitionDelay: isMobile ? "0.05s" : "0.15s" }}>
                <TextScramble text="We craft brands" delay={200} />
              </span>
            </span>
            <span className={`block transition-all duration-700 ease-out ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-full"}`} style={{ transitionDelay: isMobile ? "0.1s" : "0.3s" }}>
          ...
          <p className={`text-white/70 font-body font-medium text-sm sm:text-base leading-relaxed mb-6 sm:mb-8 max-w-md transition-all duration-700 ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`} style={{ transitionDelay: isMobile ? "0.15s" : "0.5s" }}>
            20 specialists. 18 disciplines. 1500+ clients across design, development, and financial operations.
          </p>

          <div className={`flex flex-col sm:flex-row items-stretch sm:items-start gap-3 transition-all duration-700 ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`} style={{ transitionDelay: isMobile ? "0.2s" : "0.65s" }}>
            <Link
              to="/work"
              className="magnetic-btn bg-accent text-accent-foreground font-display font-bold text-sm sm:text-base px-5 sm:px-6 py-3 rounded-full inline-flex items-center justify-center gap-2 sm:gap-3 hover:shadow-[0_0_40px_hsl(var(--accent)/0.3)] group transition-all duration-500 ease-out min-h-12 w-full sm:w-auto"
              data-cursor-hover
            >
              Our Work
            ...
            <button
              ...
              className="magnetic-btn border border-white/20 text-white font-display text-sm sm:text-base px-5 sm:px-6 py-3 rounded-full inline-flex items-center justify-center gap-2 sm:gap-3 hover:border-accent hover:text-accent transition-all duration-500 ease-out min-h-12 w-full sm:w-auto"
              data-cursor-hover
            >
              Start a Project
            </button>
      ...
      <div className={`relative z-10 mt-auto transition-all duration-700 ${loaded ? "opacity-100" : "opacity-0"}`} style={{ transitionDelay: isMobile ? "0.25s" : "0.6s" }}>
        ...
        <div className="flex items-center py-3 sm:py-5">
          ...
          <MarqueeStrip speed={isMobile ? 90 : 60}>
