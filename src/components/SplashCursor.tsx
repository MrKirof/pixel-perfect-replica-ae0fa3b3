...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function handleMouseMove(e: MouseEvent) {
      ...
      if (pointerRef.current) {
        pointerRef.current.style.transform = `translate(${e.clientX}px, ${e.clientY}px)`;
      }
      const pointer = pointers[0];
      const posX = scaleByPixelRatio(e.clientX);
      const posY = scaleByPixelRatio(e.clientY);
  ...
  return (
    <>
      ...
      >
        <div className="w-4 h-4 -ml-2 -mt-2 rounded-full border-2 border-accent bg-accent/20 backdrop-blur-sm shadow-[0_0_12px_hsl(var(--accent)/0.5)]" />
      </div>
      {/* Fluid canvas */}
