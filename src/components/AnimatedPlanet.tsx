...
const AnimatedPlanet = ({
  ...
  useEffect(() => {
    ...
    const camera = new THREE.PerspectiveCamera(45, container.offsetWidth / container.offsetHeight, 0.1, 200);
    ...
    const planetGeo = new THREE.SphereGeometry(planetRadius, sphereDetail, sphereDetail);
    const planetMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float b1 = step(0.15, bandNorm) * (1.0 - step(0.30, bandNorm));
          float b2 = step(0.40, bandNorm) * (1.0 - step(0.55, bandNorm));
          float b3 = step(0.65, bandNorm) * (1.0 - step(0.80, bandNorm));
          float b4 = step(0.88, bandNorm);
          float brightMask = max(max(b1, b2), max(b3, b4));
          float baseLuma = mix(0.02, 0.55, brightMask);
          ...
          vec3 darkTint = vec3(0.12, 0.06, 0.03);
          vec3 brightTint = vec3(0.55, 0.42, 0.28);
          ...
          vec3 color = baseColor * lighting + vec3(spec) + rimColor;
    ...
    // Atmosphere
    const atmosGeo = new THREE.SphereGeometry(planetRadius * 1.025, Math.min(sphereDetail, 64), Math.min(sphereDetail, 64));
    ...
    // Ring rocks — use InstancedMesh instead of individual meshes
    const ringInner = planetRadius * 1.3;
    const ringOuter = planetRadius * 2.5;
    const ringRockGeo = new THREE.IcosahedronGeometry(0.05, 0);
    ...
    const ringMesh = new THREE.InstancedMesh(ringRockGeo, ringRockMat, ringRockCount);
    ringMesh.visible = false;
    scene.add(ringMesh);

    const ringAngles = new Float32Array(ringRockCount);
    const ringRadii = new Float32Array(ringRockCount);
    const ringSpeeds = new Float32Array(ringRockCount);
    const ringYOffsets = new Float32Array(ringRockCount);
    const ringScales = new Float32Array(ringRockCount);
    const dummy = new THREE.Object3D();

    for (let i = 0; i < ringRockCount; i++) {
      ringAngles[i] = Math.random() * Math.PI * 2;
      ringRadii[i] = ringInner + Math.random() * (ringOuter - ringInner);
      ringSpeeds[i] = 0.0003 + Math.random() * 0.0008;
      ringYOffsets[i] = (Math.random() - 0.5) * 0.6;
      const isLarge = Math.random() < 0.08;
      ringScales[i] = isLarge ? 1.5 + Math.random() * 2.5 : 0.4 + Math.random() * 1.2;
    }

    // Background asteroids
    const asteroidCount = 15;
    const asteroidGeo = new THREE.IcosahedronGeometry(0.15, 0);
    const asteroidMat = new THREE.MeshStandardMaterial({ color: 0x665544, roughness: 0.9, metalness: 0.1 });
    const asteroidMesh = new THREE.InstancedMesh(asteroidGeo, asteroidMat, asteroidCount);
    scene.add(asteroidMesh);
    const asteroidData: { x: number; y: number; z: number; rx: number; ry: number; drift: number; scale: number }[] = [];
    for (let i = 0; i < asteroidCount; i++) {
      ...
      dummy.position.set(asteroidData[i].x, asteroidData[i].y, asteroidData[i].z);
    ...
    // Falling stars
    const fireCount = fallingStarCount;
    ...
    const randomStarStart = (): THREE.Vector3 =>
      new THREE.Vector3((Math.random() - 0.3) * 60, 18 + Math.random() * 20, -3 - Math.random() * 25);
    ...
    for (let i = 0; i < fireCount; i++) {
      ...
      for (let j = 0; j < trailPerStar; j++) tHistory.push(startPos.clone());
      const angle = -0.6 - Math.random() * 0.4;
      const vel = new THREE.Vector3(Math.cos(angle) * -0.7, Math.sin(angle) * -1.0, (Math.random() - 0.5) * 0.1).normalize();
      fallingStars.push({
        position: startPos.clone(), velocity: vel, speed: 0.08 + Math.random() * 0.12,
        trailGeo: tGeo, trailPositions: tPos, trailAlphas: tAlpha, trailSizes: tSize,
        trailHistory: tHistory, trailHead: 0, trailPoints: tPoints, brightness: 0.5 + Math.random() * 0.5,
      });
    }
    ...
    const planetRestY = planet.position.y;
    const planetStartY = planetRestY + 35;
    ...
    const ringScatterRadii = new Float32Array(ringRockCount);
    for (let i = 0; i < ringRockCount; i++) {
      ringScatterRadii[i] = ringRadii[i] * (3.0 + Math.random() * 3.0);
    }

    const startZ = 28, endZ = 20, startCamY = -3, endCamY = 0, zoomDuration = 4.5;
    camera.position.set(0, startCamY, startZ);

    // Throttle: update every other frame on mid-tier
    let frameCount = 0;
    ...
    const animate = () => {
      ...
      // Planet entrance
      const planetStart = 1.0, planetEnd = 3.0;
      ...
      // Ring rocks entrance and orbit
      const ringStart = 2.5, ringEnd = 4.5;
      if (time >= ringStart && ringRockCount > 0) {
        ...
        for (let i = 0; i < ringRockCount; i++) {
          ...
          const currentRadius = ringT < 1
            ? ringScatterRadii[i] + (ringRadii[i] - ringScatterRadii[i]) * ringEase
            : ringRadii[i];
          const a = ringAngles[i];
          dummy.position.set(
            Math.cos(a) * currentRadius + planet.position.x,
            ringYOffsets[i] + planet.position.y,
            Math.sin(a) * currentRadius + planet.position.z
          );
          dummy.scale.setScalar(ringScales[i] * ringEase);
      ...
      for (let i = 0; i < fireCount; i++) {
        ...
        if (fs.position.y < -25) {
          ...
          for (let j = 0; j < trailPerStar; j++) fs.trailHistory[j].copy(newStart);
          fs.trailHead = 0;
        }
    ...
    return () => {
      ...
      fallingStars.forEach(fs => fs.trailGeo.dispose());
      if (container.contains(renderer.domElement)) {
        container.removeChild(renderer.domElement);
      }
    };
  }, [ringRockCount, dustCount, fallingStarCount, sphereDetail, pixelRatio]);
